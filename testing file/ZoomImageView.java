...
public class ZoomImageView extends View {
    ...
    public void setImageBitmap(Bitmap bitmap) {
        sourceBitmap = bitmap;
        invalidate();
        Log.d("sourceBitmap", "Ingress sourceBitmap");
    ...
    public boolean onTouchEvent(MotionEvent event) {
        Log.d("onTouchEvent", "Ingress onTouchEvent");
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                //Log.d("ACTION_MOVE", "Ingress ACTION_MOVE");
                if (event.getPointerCount() == 1) {
                    Log.d("one finger", "Ingress one finger");
                    // 只有单指按在屏幕上移动时，为拖动状态
                    float xMove = event.getX();
                    float yMove = event.getY();
                    Log.v("axis",  + xMove +" " + yMove);
    ...
    private void initBitmap(Canvas canvas) {
        Log.v("initBitmap",  "initBitmap");
        if (sourceBitmap != null) {
            matrix.reset();
            int bitmapWidth = sourceBitmap.getWidth();
            int bitmapHeight = sourceBitmap.getHeight();
            Log.v("bitmapWidth,bitmapHeight",  bitmapWidth + " " + bitmapHeight);
