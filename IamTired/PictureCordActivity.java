...
public class PictureCordActivity extends AppCompatActivity {
    ImageView imageView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.picture_cord_layout);
        ...
        Uri imagePath = getIntent().getParcelableExtra("imagePath");
        Bitmap bitmapImage;
        try
        {
        ...
        button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                try {
                    rateWidth = (float) bitmap.getWidth() / imageView.getWidth();
                    rateHeight = (float) bitmap.getHeight() / imageView.getHeight();
                } catch (ArithmeticException e) {
                    Log.e("source", "width: " + imageView.getWidth() + " height: " + imageView.getHeight());
                    System.exit(1);
                }
                if (flag) {
                    topLeft.y -= screenHeight / 2 - imageView.getHeight() / 2;
                    topRight.y-= screenHeight / 2 - imageView.getHeight() / 2;
                    bottomLeft.y -= screenHeight / 2 - imageView.getHeight() / 2; //直的
                    bottomRight.y -= screenHeight / 2 - imageView.getHeight() / 2;
                }
                else {
                    topLeft.x -= screenWidth / 2 - imageView.getWidth() / 2;//橫的
                    topRight.x -= screenWidth / 2 - imageView.getWidth() / 2;//橫的
                    bottomLeft.x -= screenWidth / 2 - imageView.getWidth() / 2;//橫的
                    bottomRight.x -= screenWidth / 2 - imageView.getWidth() / 2;//橫的
                }
                Log.e("flag", ""+flag);
